...
// the master process reads the file and calculates the occurrence of the patterns on the borders ( critical region )
// since the access of every process is limited to its own local memory
...
printf( "Approximate Pattern Mathing: "
  ...
  // iterate over processes by rank to find the borders ( array limits )in each block of data

  for (int r=1;r<comm_size;r++)
  {
    char limits[2*p_size - 2 ];
    int k=0;
    for (int j=p_size - 1 ;j>0;j-- )
    {
      limits[k]=buf[(n_bytes/comm_size)*r - j];
      k++;
    }
    ...
    for ( int j = 0 ; j < 2*p_size -2 ; j++ )
    {
      ...
      if ( 2*p_size - 2  - j < p_size  )
      {
       ...
       int distance = levenshtein( pattern[i], &limits[j], size, columnj ) + p_size - size ;
...
free(columnj);
  ...
  //broacast the size of file to all processes
